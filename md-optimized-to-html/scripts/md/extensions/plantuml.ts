...
function validateServerUrl(url: string): string {
  try {
    ...
    if (!ALLOWED_SERVER_HOSTS.includes(parsed.hostname)) {
      throw new Error(`PlantUML server "${parsed.hostname}" is not allowed. Allowed: ${ALLOWED_SERVER_HOSTS.join(', ')}`)
    }
    return url
  } catch (e) {
    if (e instanceof TypeError) throw new Error(`Invalid PlantUML server URL: ${url}`)
    throw e
  }
}
...
export function markedPlantUML(options: PlantUMLOptions = {}): MarkedExtension {
  const resolvedOptions: Required<PlantUMLOptions> = {
    serverUrl: validateServerUrl(options.serverUrl || `https://www.plantuml.com/plantuml`),
