...
function validateBaseUrl(url: string): string {
  try {
    ...
    if (!parsed.hostname.endsWith(".googleapis.com")) {
      throw new Error(`Base URL hostname "${parsed.hostname}" is not allowed. Expected: *.googleapis.com`);
    }
    return url;
  } catch (e) {
    if (e instanceof TypeError) throw new Error(`Invalid GOOGLE_BASE_URL: ${url}`);
    throw e;
  }
}
...
function getGoogleBaseUrl(): string {
  const base = validateBaseUrl(process.env.GOOGLE_BASE_URL || "https://generativelanguage.googleapis.com");
