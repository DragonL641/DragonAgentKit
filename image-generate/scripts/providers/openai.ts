...
const ALLOWED_HOSTS = ["api.openai.com"];

function validateBaseUrl(url: string, defaultHost: string): string {
  try {
    const parsed = new URL(url);
    if (!ALLOWED_HOSTS.includes(parsed.hostname) && parsed.hostname !== new URL(`https://${defaultHost}`).hostname) {
      throw new Error(`Base URL hostname "${parsed.hostname}" is not allowed. Expected: ${defaultHost}`);
    }
    return url;
  } catch (e) {
    if (e instanceof TypeError) throw new Error(`Invalid OPENAI_BASE_URL: ${url}`);
    throw e;
  }
}
...
export async function generateImage(
  ...
  const baseURL = validateBaseUrl(rawBaseURL, "api.openai.com");
