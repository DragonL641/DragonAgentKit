...
function validateBaseUrl(url: string): string {
  try {
    ...
    if (!parsed.hostname.endsWith(".aliyuncs.com")) {
      throw new Error(`Base URL hostname "${parsed.hostname}" is not allowed. Expected: *.aliyuncs.com`);
    }
    return url;
  } catch (e) {
    if (e instanceof TypeError) throw new Error(`Invalid DASHSCOPE_BASE_URL: ${url}`);
    throw e;
  }
}
...
function getBaseUrl(): string {
  const base = validateBaseUrl(process.env.DASHSCOPE_BASE_URL || "https://dashscope.aliyuncs.com");
...
export async function generateImage(
  ...
  if (args.referenceImages.length > 0) {
    throw new Error(
      "Reference images are not supported with DashScope provider in image-generate. Use --provider google with a Gemini multimodal model."
    );
  }
