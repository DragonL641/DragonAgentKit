...
Options:
  ...
  --provider google|openai|dashscope|glm  Force provider (auto-detect by default)
  -m, --model <id>          Model ID
...
Environment variables:
  ...
  GLM_IMAGE_MODEL           Default GLM model (glm-image)
  OPENAI_BASE_URL           Custom OpenAI endpoint
  GOOGLE_BASE_URL           Custom Google endpoint
  DASHSCOPE_BASE_URL        Custom DashScope endpoint
  GLM_BASE_URL              Custom GLM endpoint

Env file load order: CLI args > EXTEND.md > process.env > .dragonskills/image-generate.env`);
}
...
function parseArgs(argv: string[]): CliArgs {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    if (a === "--provider") {
      const v = argv[++i];
      if (v !== "google" && v !== "openai" && v !== "dashscope" && v !== "glm") throw new Error(`Invalid provider: ${v}`);
      out.provider = v;
      continue;
    }
...
const ALLOWED_ENV_PREFIXES = ["OPENAI_", "GOOGLE_", "GEMINI_", "DASHSCOPE_", "GLM_", "IMAGE_"];

async function loadEnv(): Promise<void> {
  // Resolve project root: main.ts -> scripts/ -> image-generate/ -> project root
  const scriptDir = path.dirname(new URL(import.meta.url).pathname);
  const projectRoot = path.resolve(scriptDir, "..", "..");
  const envPath = path.join(projectRoot, ".dragonskills", "image-generate.env");
  const env = await loadEnvFile(envPath);

  for (const [k, v] of Object.entries(env)) {
    if (!ALLOWED_ENV_PREFIXES.some((prefix) => k.startsWith(prefix))) continue;
  ...
  for (const line of lines) {
    ...
    if (trimmed.includes(":") && !trimmed.startsWith("-")) {
      ...
      if (key === "version") {
        ...
        config.default_model = { google: null, openai: null, dashscope: null, glm: null };
        currentKey = "default_model";
      } else if (currentKey === "default_model" && (key === "google" || key === "openai" || key === "dashscope" || key === "glm")) {
        const cleaned = value.replace(/['"]/g, "");
...
async function loadExtendConfig(): Promise<Partial<ExtendConfig>> {
  ...
  const paths = [
    path.join(cwd, ".dragonskills", "image-generate", "EXTEND.md"),
    path.join(home, ".dragonskills", "image-generate", "EXTEND.md"),
  ];
...
function detectProvider(args: CliArgs): Provider {
  ...
  const available = [hasGoogle && "google", hasOpenai && "openai", hasDashscope && "dashscope", hasGlm && "glm"].filter(Boolean) as Provider[];

  if (available.length === 1) return available[0]!;
  if (available.length > 1) return available[0]!;

  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, DASHSCOPE_API_KEY, or GLM_API_KEY.\n" +
...
async function loadProviderModule(provider: Provider): Promise<ProviderModule> {
  ...
  if (provider === "glm") {
    return (await import("./providers/glm")) as ProviderModule;
  }
...
async function main(): Promise<void> {
  ...
  if (!model && extendConfig.default_model) {
    ...
    if (provider === "glm") model = extendConfig.default_model.glm ?? null;
  }
