...
"""
Standalone entry point for stock-US-morning-brief.
Replaces Claude Code orchestration with pure Python + Claude API for summary generation.

Usage:
    uv run run.py [--dry-run] [--skip-summary] [--log-level LEVEL]
"""
...
DRAGONSKILLS_DIR = Path(__file__).resolve().parents[1] / ".dragonskills"
ENV_FILE = DRAGONSKILLS_DIR / "stock-US-morning-brief.env"
CONFIG_FILE = DRAGONSKILLS_DIR / "stock-US-morning-brief.json"
...
def main():
    parser = argparse.ArgumentParser(description="Stock US Morning Brief - Standalone Runner")
    ...
    logger.info("=" * 60)
    logger.info("Stock US Morning Brief - Starting")
