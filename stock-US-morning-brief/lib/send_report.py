...
def render_template(template, data, language, recipient_settings):
    ...
    # Market sections - US only
    market_sections_html = ''
    us_data = data.get('us', {})
    if us_data:
        try:
            provider = USMarketProvider()
            market_sections_html = provider.render_section(us_data, config)
        except Exception as e:
            print(f'WARNING: Failed to render US market: {e}')

    html = html.replace('{{market_sections}}', market_sections_html)
