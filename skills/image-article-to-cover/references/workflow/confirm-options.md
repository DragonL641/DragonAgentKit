...
| `quick_mode: true` in preferences | Type, Palette, Rendering, Text, Mood, Font | **Aspect Ratio** (unless `--aspect` specified) |
| Otherwise | None | All 7 questions |

**Important**: Aspect ratio is ALWAYS asked unless explicitly specified via `--aspect` CLI flag. User presets in preferences are shown as recommended option, not auto-selected.
...
**When output dir needs asking** (no `default_output_dir` in preferences + file path input):
