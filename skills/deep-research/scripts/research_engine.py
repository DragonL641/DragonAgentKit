...
class ResearchMode(Enum):
    ...
    STANDARD = "standard"  # 6 phases: skip plan, critique, refine
...
## Execute:
1. First check if httpx is available: `python -c "import httpx"`
2. If available, use built-in search engine via Bash:
   - `python -m scripts.search.cli "query" --mode <mode> -c <count>`
   - Modes: auto, general, news, academic, deep, people
3. If httpx not installed, fall back to WebSearch
4. Use WebFetch to deep-dive into 5-10 most promising sources
...
## Tools to Use:
- Bash + scripts.search.cli: Primary multi-provider search
- WebSearch: Fallback if search engine unavailable
...
Examples:
  ...
  python research_engine.py -q "renewable energy investment trends" -m ultradeep
        """
    )
    ...
    parser.add_argument(
        '--mode', '-m',
        type=str,
        choices=['standard', 'deep', 'ultradeep'],
        default='standard',
