...
|----------|-------------|---------|
| Output directory | No `default_output_dir` in preferences | `{article-dir}/`, `{article-dir}/imgs/` (Recommended), `{article-dir}/illustrations/`, `illustrations/{topic-slug}/` |
...
### 1.5 Load Preferences ⛔ BLOCKING

**CRITICAL**: If preferences not found, MUST complete first-time setup before ANY other questions or steps. Do NOT proceed to reference images, do NOT ask about content, do NOT ask about type/style — ONLY complete the preferences setup first.

```bash
test -f .dragonskills/image-article-to-illustration/EXTEND.md && echo "project"
test -f "$HOME/.dragonskills/image-article-to-illustration/EXTEND.md" && echo "user"
```

| Result | Action |
|--------|--------|
| Found | Read, parse, display summary → Continue |
| Not found | ⛔ **BLOCKING**: Run first-time setup ONLY ([config/first-time-setup.md](config/first-time-setup.md)) → Complete and save preferences → Then continue |
...
### Q3: Style ⚠️ REQUIRED (ALWAYS ask, even with preferred_style in preferences)

If preferences have `preferred_style`:
...
If no `preferred_style` in preferences (present Core Styles first):
...
### Q4: Image Text Language ⚠️ REQUIRED when article language differs from preferences `language`

Detect article language from content. If different from preferences `language` setting, MUST ask:
- Article language (match article content) (Recommended)
- Preferences language (user's general preference)

**Skip only if**: Article language matches preferences `language`, or preferences have no `language` setting.
...
|------------------------|--------|
| Yes (e.g., image-generate with Google) | Pass reference images via `--ref` |
