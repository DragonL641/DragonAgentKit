...
| Logo 设计 | 类似 image-brand-to-logo: 收集品牌信息 → 选风格 → 10个变体 → 出图 → 迭代 |
| 分镜故事 | 类似 image-story-to-storyboard: 润色故事 → 拆分镜 → 选风格 → 批量出图 |
...
将 `${SKILL_DIR}/../image-generate/scripts/main.ts` 复制到新 Skill 的 scripts/ 目录。
