...
Use Bash to check preferences file existence (priority order):

```bash
# Check project-level first
test -f .dragonskills/image-story-to-comic/EXTEND.md && echo "project"

# Then user-level (cross-platform: $HOME works on macOS/Linux/WSL)
test -f "$HOME/.dragonskills/image-story-to-comic/EXTEND.md" && echo "user"
```

| Path | Location |
|------|----------|
| `.dragonskills/image-story-to-comic/EXTEND.md` | Project directory |
| `$HOME/.dragonskills/image-story-to-comic/EXTEND.md` | User home |

**When Preferences Found** → Read, parse, **output summary to user**:
...
**When Preferences Not Found** → First-time setup:
...
3. Create preferences file at user-chosen location
4. Confirm: "✓ Preferences saved to [path]"

**Preferences Supports**: Watermark | Preferred art/tone/layout | Custom style definitions | Character presets | Language preference

Schema: `config/preferences-schema.md`

**Important**: Once preferences exist, watermark, language, and style defaults are NOT asked again in Confirmation 1 or 2. These are session-persistent settings.
...
5. **Determine language**:
   - If preferences have `language` → use it
...
**Note**: Watermark and language already configured in preferences (Step 1).
...
- Language: [from preferences or detected]
...
**Strategy A: Using `--ref` parameter** (e.g., image-generate)

```bash
# Each page generation MUST include --ref
npx -y bun ${SKILL_DIR}/../image-generate/scripts/main.ts \
