...
If CJK content detected AND `md-cjk-layout-optimize` skill is available:
...
**If user agrees**: Invoke `md-cjk-layout-optimize` skill to format the file, then use formatted file as input.
...
Custom configurations via preferences. Supports: Default theme | Custom CSS variables | Code block style
