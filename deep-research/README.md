...
| Tool | Priority | Setup |
|------|----------|-------|
| Built-in search engine | **Primary** — multi-provider aggregation | `pip install httpx` + API keys |
| WebSearch | Fallback — if search engine fails or rate-limited | None (built-in) |
| Exa MCP | Optional — semantic/neural search alongside search engine | MCP config |
